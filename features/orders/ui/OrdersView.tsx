...
export default function OrdersView() {
  ...
  const { data: orders = [], isLoading, refetch, isFetching } = useQuery({
    ...
    queryFn: () => getOrders(statusFilter !== "all" ? { status: statusFilter } : undefined),
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              ...
              {pendingCount > 0 && (
                <span className="ml-2 inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-yellow-500/20 text-yellow-500 text-xs font-semibold">
        ...
        <div className="flex gap-2 mb-6 flex-wrap">
          <button
            ...
            className={`px-4 py-2 rounded-xl text-sm font-medium border transition-all ${statusFilter === "all"
          ...
          {(["pending", "confirmed", "preparing", "ready"] as OrderStatus[]).map((s) => {
            ...
            return (
              <button
                ...
                className={`px-4 py-2 rounded-xl text-sm font-medium border transition-all ${statusFilter === s
                  ? `${cfg.color}`
                  : "bg-muted/20 text-muted-foreground border-border hover:bg-muted/40"
                  }`}
              >
