  ...
  return (
    <div className="relative overflow-hidden">
      <div
        ...
        className="relative z-10 flex items-center gap-3 p-3 bg-transparent"
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => onChangeQty(s.item.id, s.qty - 1)} className="w-8 h-8 rounded bg-muted/20 flex items-center justify-center">-</button>
          <div className="w-8 text-center">{s.qty}</div>
          <button onClick={() => onChangeQty(s.item.id, s.qty + 1)} className="w-8 h-8 rounded bg-muted/20 flex items-center justify-center">+</button>
          <div className="w-20 text-right font-medium">${(s.item.price * s.qty).toFixed(2)}</div>
        </div>
      </div>

      <div className="absolute inset-y-0 right-0 w-24 flex items-center justify-center text-muted-foreground pointer-events-none">
        <X className="w-5 h-5" />
      </div>
    </div>
  )
